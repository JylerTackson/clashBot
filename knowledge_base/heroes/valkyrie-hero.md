---
name: Heroic Valkyrie
slug: valkyrie-hero
base_card: Valkyrie
base_card_slug: valkyrie
base_card_file: cards/valkyrie.md
rarity: Rare
elixir_cost: 4
ability_name: Wild Whirlwind
ability_cost: 3
total_elixir_cost: 7
card_type: Troop
targets: ground
source_url: "https://clashroyale.fandom.com/wiki/Valkyrie/Hero"
image_path: heroes/images/valkyrie-hero.png
scraped_at: "2026-09-02T02:48:37+00:00"
release_date: August 3 2026
---

# Heroic Valkyrie

> *In-game description:* "Whirl Power! Hero Valkyrie has gone round and round the Arena, getting whipped into shape and chopping down troops with an axe that wont quit. Now, she's ready for her final test at the gates of Valkhalla."

**Base card:** [valkyrie.md](../cards/valkyrie.md)

## Overview

The Heroic Valkyrie is a Rare card that is unlocked with 200 hero fragments.

- m.

## Ability: Wild Whirlwind

*Spins rapidly, dealing damage and increasing her movement speed while taking less damage.* (costs 3 Elixir; 4 + 3 = 7 total).

## Attributes

**Attributes**

| Cost | Hit Speed | First Hit Speed | Speed | Deploy Time | Range | Splash Radius | Target | Count | Transport | Type | Rarity |
|---|---|---|---|---|---|---|---|---|---|---|---|
| 4 | 1.5 sec | 0.1 sec | Medium (60) | 1 sec | Melee: Medium (1.2) | 2 | Ground | x1 | Ground | Troop | Rare |

**Attributes**

| Cost | Hit Speed | Crown Tower Damage | Speed | Duration | Radius | Damage reduction | Dash Distance |
|---|---|---|---|---|---|---|---|
| 3 | 0.25 sec | -50% | Medium (60) | 3.5 sec | 2.5 | 15% | 5.5 |

## Stats by level

**Card Statistics**

| Level | Hitpoints | Area Damage | Damage Per Second | Ability Damage |
|---|---|---|---|---|
| 3 | 890 | 124 | 82 | 45 |
| 4 | 979 | 137 | 91 | 50 |
| 5 | 1076 | 150 | 100 | 55 |
| 6 | 1184 | 165 | 110 | 60 |
| 7 | 1303 | 182 | 121 | 66 |
| 8 | 1433 | 200 | 133 | 73 |
| 9 | 1576 | 220 | 146 | 80 |
| 10 | 1734 | 242 | 161 | 88 |
| 11 | 1907 | 266 | 177 | 97 |
| 12 | 2098 | 293 | 195 | 107 |
| 13 | 2307 | 322 | 214 | 117 |
| 14 | 2538 | 354 | 236 | 129 |
| 15 | 2792 | 389 | 259 | 142 |
| 16 | 3071 | 428 | 285 | 156 |

## Strong against

- Clumped ground troops — her normal attack is Ground-only splash with a 2 tile radius, and Wild Whirlwind widens that to a 2.5 tile radius spin.
- Ground pushes she can chase down — the ability raises her movement speed and gives a 5.5 tile dash distance to reach targets.
- No further matchups are specified on the source page.

## Weak against

- Air troops — she targets Ground only, so she cannot hit them with her axe or her ability.
- Crown Towers — her ability deals -50% Crown Tower damage, so it is a defensive tool rather than a Tower finisher.
- No further counters are specified on the source page.

## Notes / synergies

- Count her as a 7 Elixir card for trade math: 4 for the deploy plus 3 for Wild Whirlwind.
- Base statline: 4 Elixir, 1.5 sec hit speed, a very fast 0.1 sec first hit, Medium (60) speed, Melee: Medium (1.2) range and a 2 tile splash radius, Ground targets only.
- Wild Whirlwind makes her spin rapidly for 3.5 seconds, hitting every 0.25 sec in a 2.5 tile radius with a 5.5 tile dash distance and Medium (60) movement.
- During the ability she takes 15% reduced damage and moves faster, but she can still be killed while spinning.
- Ability damage is a separate, smaller per-tick value (for example 97 at Level 11 versus 266 area damage), so its strength comes from the 0.25 sec tick rate over 3.5 seconds.
- The ability deals -50% damage to Crown Towers.
- The source page has no Strategy section, so no further usage guidance is specified.

## Source

- https://clashroyale.fandom.com/wiki/Valkyrie/Hero (scraped 2026-09-02T02:48:37+00:00)
