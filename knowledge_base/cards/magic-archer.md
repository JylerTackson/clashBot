---
name: Magic Archer
slug: magic-archer
rarity: Legendary
elixir_cost: 4
card_type: Troop
targets: ground_and_air
has_evolution: false
has_hero_variant: true
source_url: "https://clashroyale.fandom.com/wiki/Magic_Archer"
image_path: cards/images/magic-archer.png
scraped_at: "2026-09-02T02:35:11+00:00"
hero_file: heroes/magic-archer-hero.md
arena: "Rascal's Hideout"
release_date: 2 March 2018
---

# Magic Archer

> *In-game description:* "Not quite a Wizard, nor an Archer - he shoots a magic arrow that passes through and damages all enemies in its path. It's not a trick, it's magic!"

**Related:** [Hero variant: magic-archer-hero](../heroes/magic-archer-hero.md)

## Overview

The Magic Archer is a Legendary card that is unlocked from the Rascal's Hideout (Arena 13). It spawns an area damage, air-targeting, long-ranged, ground troop with moderate hitpoints and moderate damage. The Magic Archer launches an attack that travels up to its maximum range, while piercing through enemies. He takes the appearance of a man with gray hair with white stripes, glowing turquoise eyes, bushy black eyebrows, and a French-fork beard, wears a dark green tunic, a team-colored cape, a brown belt with a triangular copper rune and fingerless gloves, and dark brown pants and boots, and wields a glowing team-colored bow. A Magic Archer card costs 4 Elixir to deploy.

<!-- Card Overviews summary (wiki) -->
An inspiring idol for Archers (besides their Queen)! For 4 Elixir, he has moderate health, damage and the ability to send his arrow even further than his targeting range. He can safely pierce buildings from afar, as well as multiple targets within the arrow's linear path, and can easily clear swarms. However, tougher troops and heavy spells will easily destroy him.

If his **Hero** is active, you can spend 2 Elixir to use his **Triple Pierce** ability, causing him to teleport back a few tiles, leave a stationary decoy that has moderate hitpoints, and enhance the next attack to fire three arrows at once. Although the three arrows don’t do increased damage, they have an incredibly long range and extended width for a bigger covering. Guess all those lessons with the Boss Bandit have been finally paying out!

## Attributes

**Attributes**

| Cost | Hit Speed | First Hit Speed | Speed | Deploy Time | Range | Projectile Range | Projectile Width | Projectile Speed | Target | Count | Transport | Type | Rarity |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 4 | 1.1 sec | 0.7 sec | Medium (60) | 1 sec | 7 | 11 | 0.5 | 1000 | Air & Ground | x1 | Ground | Troop | Legendary |

## Stats by level

**Card Statistics**

| Level | Hitpoints | Area Damage | Damage per second |
|---|---|---|---|
| 9 | 437 | 110 | 99 |
| 10 | 481 | 121 | 109 |
| 11 | 529 | 133 | 120 |
| 12 | 582 | 146 | 132 |
| 13 | 640 | 161 | 146 |
| 14 | 704 | 177 | 160 |
| 15 | 775 | 195 | 177 |
| 16 | 852 | 214 | 194 |

## Abilities and special mechanics

Not specified on source page

## Strong against

- Defensive buildings in enemy territory - he outranges Bomb Tower, Cannon, Tesla and Inferno Tower and survives The Log, Zap, Giant Snowball and Arrows.
- Spawner buildings - his piercing arrow hits the building and the spawned troops at once, and the finishing hit also wipes the death spawn; especially strong versus Tombstone and Goblin Hut, whose troops he one-shots.
- Mortar and X-Bow - he can snipe them from around the bridge, often clipping the Crown Tower in the same shot.
- Swarms - Bats, Goblin Gang, Guards and Skeleton Army fall to his fast, piercing shots; with a Crown Tower he destroys a Minion Horde without taking damage.
- Graveyard - rapid piercing shots hit multiple Skeletons at once.
- Goblin Barrel - placed so he distracts one Goblin while one shot pierces the top two.
- Skeleton Barrel - deployed behind your King's Tower in the same lane when the barrel's shadow is 5 tiles from the Crown Tower, one arrow pierces almost all the Skeletons.
- Lava Hound - his high attack speed kills it and his splash handles the Lava Pups; a well-timed fatal hit catches all Pups at once.
- Medium-range troops - Wizard, Executioner, Hunter and Bowler can be shot from about 7 tiles (outside their 5.5 tile sight range) so they never fire back; the same works on Ram Rider.
- Princess at the bridge - he damages her whether or not she distracts him, chips the Tower and survives to counterattack.
- Three Musketeers when split - with The Log and correct timing he stops two of them and chips the Tower, leaving 3 Elixir for the third.

## Weak against

- High-damage spells - his low hitpoints for 4 Elixir mean Fireball or Poison kill him for a neutral trade, better if a Crown Tower is clipped.
- Lightning and Rocket - instantly obliterate him, a negative trade unless they hit something else too.
- Void - kills a lone Magic Archer in 2 hits for a positive Elixir trade.
- Barbarian Barrel - placed so the Barbarian spawns behind him, the Barbarian plus Crown Tower kill him for a 2 Elixir profit; Royal Delivery is similar.
- Mini P.E.K.K.A. - one-shots him and survives with almost all health; Elite Barbarians, Sparky and P.E.K.K.A. do the same at a negative trade.
- Valkyrie, Giant Skeleton, Mega Minion, Night Witch, Goblin Brawler, a non-jumping Mega Knight and a non-charging Prince - all 2-shot him (place them staggered so he cannot chip the Tower).
- Kiting with an Ice Golem or Goblin Giant - his very long sight range pulls him to the other lane while the Crown Tower kills him, a +2 Elixir trade.
- Spirits - an Ice, Fire or Electro Spirit placed one tile to the opposite side and five tiles up from the King's Tower kites him and his arrows miss due to their Very Fast speed.
- Swarms surrounding him - even Bats or Skeletons stop him connecting to a Tower for a +2 or +3 Elixir trade.
- Tornado plus a single troop such as an Ice Golem - drags him into King's Tower activation.
- Fisherman - hooked toward a Fisherman placed mid-field staggered 2 tiles diagonally, his piercing arrows activate the King Tower.
- Miner - 3-shots a defending Magic Archer and can be placed anywhere around him.
- Monk - a predicted ability activation reflects his arrows into his own Crown Tower.
- Poison in a Graveyard push - he is more Poison-fragile than Musketeer, Ice Wizard, Wizard or Electro Wizard.

## Notes / synergies

- Typical role: 4 Elixir long-range piercing splash support; his arrow travels to maximum range through everything in its line.
- If a defensive building sits 4 tiles from the river he can reach it without crossing the bridge; from the inner side of the bridge he can hit a passive building (larger hitbox) 5 tiles from the river, and he can snipe from the bridge in the other lane.
- Good support for tanks (Giant, Golem, Ice Golem, Elixir Golem, Lava Hound) and fast attackers (Hog Rider, Ram Rider, Battle Ram) precisely because he removes their building counters.
- Against a defensive Mortar, distract it with a mini tank in the other lane first, or it will retarget onto him when the tank enters its blind spot.
- Chip technique: line him up on the same horizontal line as an enemy troop in front of a Crown Tower so one arrow hits both; stagger him to match a staggered troop.
- Pair the chip tactic with a mini tank, since high-damage ranged troops otherwise kill him before he earns the damage.
- Like the Princess he supports pushes from the river - ground troops must walk around via the bridge, giving you time to distract them.
- Placing him by the river and 2 tiles into the opposite lane keeps him about 7 tiles from bridge-crossers, outside their sight range.
- Tornado synergy: clumping troops lets his piercing shots hit the whole group, and at the bridge he can chip the Tower at the same time; pull units slightly farther than you would for an Executioner or Wizard thanks to his range.
- Hunter synergy: the Hunter handles tanks and short-range threats while the Magic Archer answers long-range threats - watch for spells hitting both.
- Spirit synergy: an Ice, Fire or Electro Spirit alongside him adds burst on defence or Tower damage; the Ice or Electro Spirit's stun also angles swarms so one arrow hits several, key against pushes like Miner plus Minion Horde. Vulnerable to The Log or Royal Delivery.
- Second-Tower placements after one Crown Tower is down: 4 tiles from the river connects fastest but puts him in King Tower range for only 2 hits; 1 tile lower stays out of King range for a 3rd hit; the 2nd tile from the river (or 3 tiles from the river and 1 from centre) adds another hit but needs walking time; 4 tiles from the river and 3 from centre makes him target the King Tower for 5 hits.
- Stack multiple Magic Archers for Arena control - they protect each other from swarms - but keep them apart to deny spell value.
- Watch for fast friendly troops such as Goblins pushing him forward into a King-activation trap; your own Tornado can pull an enemy kiting troop away from your King's Tower.
- In 2v2, the King's Tower can be activated on him only if he is staggered toward the centre as he crosses the bridge, with the troop placed to the side of your Crown Tower.
- Even at critical health his long range and fast first hit let him land a shot before a Tower Princess arrow arrives - distract him with a cheap troop.
- An unsupported Magic Archer can be ignored by heavy decks, especially behind a Cannoneer or Dagger Duchess, which limit him to three and two hits respectively.
- Against a Graveyard, place him behind the King's Tower so spelling him risks a King activation.
- Compared to the Executioner he attacks faster, outranges him and costs 1 less Elixir, but has less health, less damage and hits fewer units.
- A Magic Archer 1 Level lower needs more hits on Princess, Firecracker, Archers, Bomber and Dart Goblin, which pierces more damage into the Tower - not recommended, as the lost hitpoints make him easy to kill past the bridge.
- If the opponent spends a spell on him, expect a follow-up card such as Barbarians that thrives without it.

## Hero variant

This card has a Hero form (Triple Threat, ability cost 2 Elixir): see [magic-archer-hero.md](../heroes/magic-archer-hero.md).

## Deck archetypes

<!-- deck-archetypes:start -->
Generated 2026-09-02T03:43:33+00:00 from 19 popular deck(s) scraped from RoyaleAPI (https://royaleapi.com/decks/popular?lang=en). Re-running Phase 2 replaces this block.

This card appears in 1 of 19 scraped decks:

- **[Bridge Spam](../archetypes/bridge-spam.md)** — 1 deck(s): [Pekka Bandit EWiz EvoGhost](../decks/bandit-battle-ram-electro-wizard-fireball-magic-archer-p-e-k-k-a-royal-ghost-zap.md)
<!-- deck-archetypes:end -->

## Creator insights (ryleycr1)

<!-- creator-insights:ryleycr1:-V4H_YeMGGk-m0.0:start -->
Match [My Opinion on the BIGGEST Balance Changes EVER!!](../matches/-V4H_YeMGGk-m0.0.md):
- clock 1:37: played at [9, 0] directly behind the King's Tower (elixir 9 -> 5) — one of three deep, safe placements ([9, 0], [2, 1] at 0:52, [16, 8] at OT 0:32) where the pierce still reaches the lane but nothing can punish him.
- OT 1:52: the river-line placement — Magic Archer label at [14, 15] on top of the Berserker already there, with Tornado cast in the same second; three of his four Tornado casts land within a few seconds of a Magic Archer, which is the clump-then-pierce pairing.
- clock 0:32 and OT 1:46: uses him as the repeat 4-elixir answer in the busiest defensive stretches (elixir 8 -> 4, then 6 -> 2), 9 deployments in all — the deck's only long-range damage.
<!-- creator-insights:ryleycr1:-V4H_YeMGGk-m0.0:end -->

<!-- creator-insights:ryleycr1:-V4H_YeMGGk-m0.1:start -->
Match [My Opinion on the BIGGEST Balance Changes EVER!!](../matches/-V4H_YeMGGk-m0.1.md):
- Eight deploy labels, all in the centre and behind his own defence: (8, 11), (8, 7), (9, 1), (8, 0), (9, 8), (8, 9), (9, 9), (4, 13), (7, 11) — he keeps a permanent centre-lane Magic Archer rather than using him as a bridge sniper.
- clock 2:37 (t=336.7): placed at (8, 11) right behind the Tornado cast five seconds earlier, so the pierced line runs through the clumped Skeletons.
- clock 0:55 (t=436.9): placed at (8, 7) inside the largest defensive stack of the match (Berserker, Archers, Royal Delivery, Goblinstein and Wall Breakers all within ten seconds) against a right-lane push of Bats, Fire Spirit and Skeletons.
<!-- creator-insights:ryleycr1:-V4H_YeMGGk-m0.1:end -->

<!-- creator-insights:ryleycr1:-V4H_YeMGGk-m0.2:start -->
Match [My Opinion on the BIGGEST Balance Changes EVER!!](../matches/-V4H_YeMGGk-m0.2.md):
- clock 0:45 at tile (2, 7), his only Magic Archer of the game: placed behind his own left tower and paired with Archers and Royal Delivery against two heavy ground units, i.e. used purely as defensive backline DPS rather than for bridge chip.
- The deploy label reads plain 'Magic Archer' and no ability is visible, so this is the standard card rather than the Hero variant.
<!-- creator-insights:ryleycr1:-V4H_YeMGGk-m0.2:end -->

<!-- creator-insights:ryleycr1:-V4H_YeMGGk-m0.3:start -->
Match [My Opinion on the BIGGEST Balance Changes EVER!!](../matches/-V4H_YeMGGk-m0.3.md):
- clock 2:28 (t=820.8) at [9, 4] and t=930.9 at [7, 0], directly behind the King Tower: both placements are deep on his own half, keeping him out of Executioner and Miner range while his pierce lines up behind the Bomb Tower.
- clock 1:12 (t=894.9, HUD 8.0 -> 4.0): played into the Royal Ghost plus Executioner defence; the enemy left tower drops 4858 -> 3552 between t=888 and t=892 off that exchange.
- As with the Berserker, there is no sign of the hero variant — all three plays read as the 4-elixir card.
<!-- creator-insights:ryleycr1:-V4H_YeMGGk-m0.3:end -->

<!-- creator-insights:ryleycr1:-V4H_YeMGGk-m0.4:start -->
Match [My Opinion on the BIGGEST Balance Changes EVER!!](../matches/-V4H_YeMGGk-m0.4.md):
- clock 0:58 (t=1106.1): Magic Archer at [8, 4] behind the Bomb Tower during the Graveyard defence - his only ranged splash, used to clear the skeleton spawn from behind the building.
- overtime 1:28 / 1:19 (t=1193.2, 1205.2): [8, 11] and [8, 9] centre placements so the pierce line covers both lanes.
- overtime 0:52 (t=1229.2): the one offensive placement, [13, 20] on the opponent's half in the right lane, five seconds after he says he is happy about the nerf.
- t=1224-1243: 'They're also nerfing the Magic Archer which I am very happy about... it was still clearly way too strong. It just seemed like they weren't going in the direction to nerf it which definitely had me upset about changes.'
<!-- creator-insights:ryleycr1:-V4H_YeMGGk-m0.4:end -->

<!-- creator-insights:ryleycr1:-V4H_YeMGGk-m1:start -->
Match [My Opinion on the BIGGEST Balance Changes EVER!!](../matches/-V4H_YeMGGk-m1.md):
- clock 2:27: Magic Archer at [8,9] right beside the centred Bomb Tower, so the two cover the same centre lane against the Electro Giant.
- clock 1:39: Magic Archer at [5,14] at the left river opens a chip engine — the enemy left tower bleeds 4527 -> 3239 over the next 16 seconds while the opponent is busy on the right.
- clock 2:21 (t=1360.2): a second Magic Archer for 4 with elixir at 6; five seconds later he interjects "Oh my god, this guy just missed an arrow."
- clock OT 1:55: replays it at [3,13] on the left in overtime as part of the cluster that takes the enemy left tower 2078 -> 1106 in six seconds.
<!-- creator-insights:ryleycr1:-V4H_YeMGGk-m1:end -->

<!-- creator-insights:ryleycr1:9tx2iYsSmMw-m0.1:start -->
Match [Ryley's BEST Games of CRL 2026](../matches/9tx2iYsSmMw-m0.1.md):
- t=200.2-203.4: Magic Archer at (3, 14) plus Valkyrie at the bridge for a full 8-elixir dump, timed so the opponent's Cannon could not be placed — "I knew that even though he had cannon in cycle and elixir for it, he couldn't really place it down in time if I just waited till the very last second".
- t=287.3: in a Magic Archer mirror he cycled his own from the very back of his half at tile (9, 1) while ahead on elixir — "I know that he wants to cycle his magic archer. But if I cycle it first and protect mine because I have the elixir advantage, my magic archer will win the 1v1 because obviously I have more elixir to support it with".
- t=257-259 and t=309-312: against the enemy Magic Archer he answers with cheap predictive spirits and Skeletons rather than a real counter — "I would just be able to predict the magic arch with my spirits", "I went for the spirit here to keep blocking his magic archer".
<!-- creator-insights:ryleycr1:9tx2iYsSmMw-m0.1:end -->

<!-- creator-insights:ryleycr1:9tx2iYsSmMw-m0.3:start -->
Match [Ryley's BEST Games of CRL 2026](../matches/9tx2iYsSmMw-m0.3.md):
- t=699-712s (opponent's card): a Magic Archer punishes his mistimed Fire Spirit and Knight — "the magic archer instantly targeted, which was very unfortunate", and costs him his right tower down to 2033.
- t=785-807s (opponent's card): he pays Poison for it every time it appears deep, and explains why the negative trade is correct — "magic archer is still most annoying when at the bridge because I can't like not deal with it at the bridge. Not only do I have to predict it, but if he just has it in cycle, I'll have to keep wasting elixir, but when he puts it low like that, I'm okay with going down, you know, one even to three elixir sometimes."
- t=778-782s and 865-869s (opponent's card): Berserker plus Bomber is his cheap physical answer — "I was worried of a mager, so I went for the berserker to block it" and "I just went for the berserker to continue blocking the magic archer just to get a second bomber on top of that."
- t=892-899s (opponent's card): criticises how the opponent used it — "For some reason this game, he dropped like literally zero magic archer at the bridge. And that's definitely not what you want to be doing. He definitely wants to be dropping magic archers at the bridge."
<!-- creator-insights:ryleycr1:9tx2iYsSmMw-m0.3:end -->

<!-- creator-insights:ryleycr1:9tx2iYsSmMw-m0.5:start -->
Match [Ryley's BEST Games of CRL 2026](../matches/9tx2iYsSmMw-m0.5.md):
- Opponent's card in this match. Ryley's default answer is a quick forward Log: "I went for the quick log here on top of the magic archer and I was already able to get a really nice situation out of this here" (1130-1134s).
- Declines a King Tower activation because the Magic Archer would not be dragged into it: "I was thinking about trying to activate the Kingar, but I felt like the march was just going to lock to the cannon anyway. So, I didn't really go for it" (1226-1231s).
- Baits it out with a bridge Mighty Miner - "I mighty mined the bridge knowing he had to go for the magic cartridge" (1307-1309s) - and finally kills it with Evo Cannon plus The Log: "I went for the evo cannon as well, just to take out that magic archer with the help of a log" (1341-1345s).
<!-- creator-insights:ryleycr1:9tx2iYsSmMw-m0.5:end -->

<!-- creator-insights:ryleycr1:X2zRpx5TN2U-m0.7:start -->
Match [This Rune Giant Deck is TOTALLY OVERPOWERED!!](../matches/X2zRpx5TN2U-m0.7.md):
- Mentioned only, in the same thought about when middle Mortar placement is correct — "I know a Magic Archer sometimes down the middle can be better, but I'm surprised that he did it without knowing my deck yet" (t=1259-1264).
<!-- creator-insights:ryleycr1:X2zRpx5TN2U-m0.7:end -->

## Source

- https://clashroyale.fandom.com/wiki/Magic_Archer (scraped 2026-09-02T02:35:11+00:00)
